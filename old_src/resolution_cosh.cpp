...
double** v_fun(int k) {
  ...
  for (int i =0;i<k;i++) {
    ...
    matrice[i][0] = 0;
    matrice[i][1] =  -V_0*1/(cosh(x_min+i*delta_x/tau)*cosh(x_min+i*delta_x/tau))/h_barre; //rentrer ici  -V/(h_barre)
  }
  return matrice;
}
...
int main () {
  ...
  for (int p=0;p<K;p++)
    ...
    fich.open("densite_cosh.dat", ios::out);
    for (int q=0;q<K;q++)
  ...
  for (int i =0;i<K;i++) {
    ...
    PyRun_SimpleString("A=loadtxt('densite_cosh.dat')");
    PyRun_SimpleString("real=A[:,1]");
    PyRun_SimpleString("imag=A[:,2]");
    //PyRun_SimpleString("proba=A[:,0]");
    PyRun_SimpleString("x=A[:,3]");
    PyRun_SimpleString("plot(x,real,label='Re')");
    PyRun_SimpleString("plot(x,imag,label='Im')");
    //PyRun_SimpleString("plot(x,proba,label='dpp')");
    PyRun_SimpleString("legend()");
