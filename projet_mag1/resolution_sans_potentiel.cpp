...
double position(complex<double> phi[K]) {
  double position= 0;
  for (int i=0;i<K;i++)
    position+=(real(phi[i])*real(phi[i]) + imag(phi[i])*imag(phi[i]))*delta_x*i*delta_x;
  return position;
}
...
double** v_fun(int k) {
  ...
  for (int i =0;i<k;i++) {
    ...
    matrice[i][0] = 0;
    matrice[i][1] = 0; //rentrer ici  -V/(h_barre)
  }
  return matrice;
}
...
int main () {
  ...
  for (int i=0;i<K;i++)
    phi_0[i]=   phi_0[i]=  pow(M_PI/2.,0.25)*(1/sqrt(a_0))*cos(k_0*(x_min+i*delta_x))*exp(-pow((x_0-(x_min+i*delta_x))/a_0,2)) + I*(pow( (M_PI/2.),0.25)*(1/sqrt(a_0))*exp(-pow((x_0-(x_min+i*delta_x))/a_0,2))*sin(k_0*(x_min+i*delta_x)));
  ...
  for (int i =0;i<K;i++) {
    ...
    PyRun_SimpleString("A=loadtxt('densite.dat')");
